...
def plot_canonical_scoring_method(rateMap, meanFr, correlationMatrix, rotations, correlations, gridScore, circularMatrix, threshold, filename, foldername): 
    ...
    plt.xlim(0, 360)
    plt.ylim(np.nanmin(correlations), 1)
    
    plt.tight_layout()
    plt.subplots_adjust(top=0.925)

    plt.savefig(foldername + '/' + filename + '_canonical.pdf', dpi=300)
    plt.close()
#     plt.show()
...
def plot_fourier_scoring(fourierSpectrogram, polarSpectrogram, maxPower, isPeriodic, filename, foldername):
    ...
    plt.savefig(foldername + '/' + filename + '_spectrograms.pdf', dpi=300)
    plt.close()
#     plt.show()
...
def plot_ring_power(averageRingPower, radii, area, maxRadius):
    # filename, 
...
def plot_ring_power_distributions(averageRingPower, radii, area, maxRadius, randomDistributionsData):
...
def plot_basic_info(rateMap, gridScore, meanFr, radii, difference, maxRadius):
